...
namespace SudokuSolver
{
    ...
    public static class SolverFactory
    {
        ...
        public static Solver CreateFromFPuzzles(string fpuzzlesURL, IEnumerable<string> additionalConstraints = null, bool onlyGivens = false)
        {
            ...
            if (fpuzzlesData.tenline != null)
            {
                foreach (var tenline in fpuzzlesData.tenline)
                {
                    foreach (var line in tenline.lines)
                    {
                        solver.AddConstraint(typeof(SumLinesConstraint), $"10;{ToOptions(line)}");
                    }
                }
            }
        ...
        public static string ToFPuzzlesURL(Solver solver, bool justBase64 = false)
        {
            ...
            List<FPuzzlesLines> tenlines = [];
            foreach (var c in solver.Constraints<SumLinesConstraint>())
            {
                if (c.sum == 10)
                {
                    string[] cells = c.cells.Select(CN).ToArray();
                    tenlines.Add(new() { lines = [cells] });
                }
            }
